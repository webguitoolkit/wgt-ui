...
/**
 * Updates the options for the current dropdown field. This allows 
...
jQuery.Dropdowner = function(input, options) {
	...
	var $input = jQuery(input).attr("dropdown", "off").addClass(options.inputClass);
	...
	var select = jQuery.Dropdowner.Select(options, input, selectCurrent, config);
	...
	function autoFill(q, sValue){
		...
		if( options.autoFill && (lastWord($input.val()).toLowerCase() == q.toLowerCase()) && lastKeyPressCode != 8 ) {
			...
			jQuery.Dropdowner.Selection(input, previousValue.length, previousValue.length + sValue.length);
		}
	};
