...
//		this.tree.logDebug("%o.render()", this);
		...
		if( this.data.draggable ){
			var aTag = jQuery( '#'+jQueryEscaped(this.span.id)+' a');
			aTag.draggable(this.data.draggable);
		}
		if( this.data.droppable ){
			var aTag = jQuery( '#'+jQueryEscaped(this.span.id)+' a');
			aTag.droppable(this.data.droppable);
		}
...
//			firstChild = this._addChildNode(new DynaTreeNode(this, this.tree, data));
			...
			firstChild.render(false,false);//fix MH
		}
	},
...
//		return false;
	...
	visitDeepFirst: function(fn, data, includeSelf) {
		// Call fn(dtnode, data) for all child nodes. Stop iteration, if fn() returns false.
		var n = 0;
		if ( this.childList )
			for (var i=0; i<this.childList.length; i++)
				n += this.childList[i].visitDeepFirst(fn, data, true);
...
DynaTree.prototype = {
	...
	initialize: function(divContainer, options) {
		...
		options = $.extend({}, $.ui.dynatree.defaults, options);
...
//  disabled: null,	
	...
	draggable: null, // if node is draggable
	droppable: null, // if node is droppable
