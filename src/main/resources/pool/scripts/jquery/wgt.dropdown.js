/*
 * dropdown - jQuery plugin 1.0 Alpha
 */

/**
 * Provide dropdown for text-inputs or textareas.
 *
 ...
 *
 * @example $("#input_box").dropdown("my_dropdown_backend.php");
 * @before <input id="input_box" />
 * @desc dropdown a text-input with remote data. For small to giant datasets.
 *
 * When the user starts typing, a request is send to the specified backend ("my_dropdown_backend.php"),
 ...
 * @option Number minChars The minimum number of characters a user has to type before the dropdownr activates. Default: 1
 * @option Number delay The delay in milliseconds the dropdownr waits after a keystroke to activate itself. Default: 400 for remote, 10 for local
 ...
 * @option Boolean matchSubset Whether or not the dropdownr can use a cache for more specific queries. This means that all matches of "foot" are a subset of all matches for "foo". Usually this is true, and using this options decreases server load and increases performance. Only useful with cacheLength settings bigger than one, like 10. Default: true
 ...
 * @option Booolean mustMatch If set to true, the dropdownr will only allow results that are presented by the backend. Note that illegal values result in an empty input box. Default: false
 * @option Object extraParams Extra parameters for the backend. If you were to specify { bar:4 }, the dropdownr would call my_dropdown_backend.php?q=foo&bar=4 (assuming the input box contains "foo"). The param can be a function that is called to calculate the param before each request. Default: none
 * @option Boolean selectFirst If this is set to true, the first dropdown value will be automatically selected on tab/return, even if it has not been handpicked by keyboard or mouse action. If there is a handpicked (highlighted) result, that result will take precedence. Default: true
 * @option Function|Boolean formatItem Provides advanced markup for an item. For each row of results, this function will be called. If false is returned, the row is skipped. Otherwise the returned value will be displayed inside an LI element in the results list. dropdownr will provide 3 parameters: the results row, the position of the row in the list of results (starting at 1), and the number of items in the list of results. Default: none, assumes that a single row contains a single value.
 ...
 * the list of dropdown items. You can use it to execute anything that does something
...
/**
 * Flush (empty) the cache of matched input's Dropdownerrs.
 *
...
/**
 * Updates the options for the current dropdown field. This allows you to change
 * things like the URL, max items to display, etc. If you're changing the URL,
 * be sure to remember to call the flushCache() method.
 * 
 * @example jQuery('input#suggest').setOptions({ max: 15 });
...
jQuery.fn.extend({
	dropdown : function(urlOrData, options) {
		...
		options = jQuery.extend({}, jQuery.Dropdowner.defaults, {
			url : isUrl ? urlOrData : null,
			data : isUrl ? null : urlOrData,
			delay : isUrl ? jQuery.Dropdowner.defaults.delay : 10
		}, options);
		...
		return this.each(function() {
			new jQuery.Dropdowner(this, options);
		});
	},
...
jQuery.Dropdowner = function(input, options) {
	...
	var $input = jQuery(input).attr("dropdown", "off").addClass(
			options.inputClass);

	var timeout;
	var previousValue = "";
	var cache = jQuery.Dropdowner.Cache(options);
	var hasFocus = 0;
	var lastKeyPressCode;
	var config = {
		mouseDownOnSelect : false
	};
	var select = jQuery.Dropdowner
	...
	$input.keydown(
			function(event) {
				...
				case options.multiple
						&& jQuery.trim(options.multipleSeparator) == ","
						&& KEY.COMMA:
				case KEY.TAB:
		...
		// having fun with opera - remove this binding and Opera submits the
		// form when we select an entry via return
	}).focus(function() {
				...
				if (!config.mouseDownOnSelect) {
					if (options.mustMatch) {
						...
						$input.search(function(result) {
							...
							if (!result) {
								$input.val("");
								$input.trigger("result", [ null, null ]);
							} else {
								$input.trigger("result", [ result.data,
										result.value ]);
							}
						});
					}
		...
		if (hasFocus++ > 1 && !select.visible() && !$input.attr('readonly')) {
			...
			.bind(
					...
					function() {
						...
						jQuery.each(trimWords($input.val()),
								function(i, value) {
									orgValue = value;
				...
				if ("data" in arguments[1]) {
					...
					function() {
						...
						request($input.val(), findValueCallback,
								findValueCallback);
						$input.focus();

						// console.timeEnd("openSelect");
	...
	/**
	 * function is used to select a value from only the value
	 */
	function selectVal(newVal) {
		function findValueCallback(q, data) {
			if (data && data.length) {
				for ( var i = 0; i < data.length; i++) {
	...
	function autoFill(q, sValue) {
		...
		if (options.autoFill
			...
			jQuery.Dropdowner.Selection(input, previousValue.length,
...
jQuery.Dropdowner.defaults = {
	inputClass : "wgtComboInput",
	resultsClass : "wgtComboResults",
	loadingClass : "wgtComboLoading",
	minChars : 1,
	delay : 400,
	matchCase : false,
	matchSubset : true,
...
jQuery.Dropdowner.Cache = function(options) {
	...
	function populate() {
		...
		jQuery.each(options.data, function(i, rawValue) {
			...
			var value = options.formatItem ? options.formatItem(rawValue,
					i + 1, options.data.length) : rawValue;
			if (value === false)
				return;
...
jQuery.Dropdowner.Select = function(options, input, select, config) {
	var CLASSES = {
		ACTIVE : "wgtComboOver"
	};
	...
	var listItems, active = -1, data, term = "";

	if (options.moreItems.length > 0)
		var moreItems = jQuery("<div>").addClass("wgtComboMoreItems").css(
				"display", "none").html(options.moreItems).appendTo(element);
	...
	function moveSelect(step) {
		active += step;
		wrapSelection();
		listItems.removeClass().slice(active, active + 1).addClass(
				CLASSES.ACTIVE);
	...
	function limitNumberOfItems(available) {
		return (options.max > 0) && (options.max < available) ? options.max
				: available;
	}
	...
	function selectItem(item) {
		if (data) {
			...
			for ( var i = 0; i < num; i++) {
				...
				if (data[i] == item) {
					active = i;
					listItems.removeClass().slice(i, i + 1).addClass(
							CLASSES.ACTIVE);
					...
					var containerTop = element.scrollTop();
					var containerBottom = containerTop + element.height();
					var elemTop = sel.get()[0].offsetTop;
					var elemBottom = elemTop + sel.height();
					if (elemTop < containerTop) {
						element.scrollTop(elemTop);
					} else if (elemBottom > containerBottom) {
						element.scrollTop(elemBottom - element.height());
					}

					// element.scrollTop( jQuery(selected).scrollTop() );
	...
	function fillList() {
		...
		for ( var i = 0; i < num; i++) {
			...
			var formatted = options.formatItem ? options.formatItem(
					data[i].data, i + 1, num, data[i].value) : data[i].value;
			if (formatted === false)
				continue;
			var style = '';
			if (data[i].value == '')
		...
		if (options.moreItems.length > 0)
			moreItems.css("display", (data.length > num) ? "block" : "none");
		list.bgiframe();
	}
	...
	return {
		...
		show : function() {
			...
			// MH if there is not enought space at the bottom paint the div at
			// top of the field (like HTML selects)
			if (scrollBottom - input.offsetHeight - offset.top < element
					.outerHeight())
				topPos = offset.top - element.outerHeight();
			element.css(
					{
						width : typeof options.width == "string"
								|| options.width > 0 ? options.width : jQuery(
								input).width() + 16,
						top : topPos,
						left : offset.left
		...
		selected : function() {
			if (listItems && listItems.filter("." + CLASSES.ACTIVE).size() > 0) {
				return data
						&& data[listItems.filter("." + CLASSES.ACTIVE).attr(
								'name')];
			}
		}
	};
...
jQuery.Dropdowner.Selection = function(field, start, end) {
